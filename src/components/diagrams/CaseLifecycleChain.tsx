        ...
        {norm.map((stage, i) => (
          <span key={stage.label} className={`flex ${hasDetail ? 'items-stretch' : 'items-center'} gap-1.5`}>
            ...
            <motion.span
              ...
              className={`rounded-md depth-edge bg-panel-2 px-2.5 py-1.5 ${
                stage.detail ? 'flex w-40 flex-col gap-1' : 'whitespace-nowrap font-mono text-2xs font-semibold text-ink'
              }`}
            >
              <span className="font-mono text-2xs font-semibold text-ink">{stage.label}</span>
              {stage.detail && <span className="text-2xs leading-snug text-muted">{stage.detail}</span>}
            </motion.span>
          </span>
        ))}
      </div>
