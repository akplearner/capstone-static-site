  ...
  return (
    <div className="space-y-4">
      ...
      {(course.roles.length === 0 || course.weeks.length === 0) && (
        <p className="text-sm text-warn">Add at least one role and one week before creating tasks.</p>
      )}

      {course.tasks.map((t, i) => {
        const isOpen = open === t.id;
        return (
          <div key={i} className="rounded-lg depth-edge">
            <div className="flex items-center justify-between gap-2 p-4">
              <button type="button" aria-expanded={isOpen} onClick={() => setOpen(isOpen ? null : t.id)} className="flex flex-1 items-center gap-2 text-left">
                ...
                <span className="text-xs text-muted">({t.role} · week {t.week} · {t.steps.length} steps)</span>
              </button>
              <button type="button" aria-label={`Remove task ${t.title}`} onClick={() => setTasks(course.tasks.filter((_, idx) => idx !== i))} className="text-muted hover:text-danger">
