...
export function InstructorGate({ children }: { children: React.ReactNode }) {
  const { unlocked, ready, unlock, lock } = useInstructorAuth();
  const [code, setCode] = useState('');
  const [error, setError] = useState(false);

  if (!ready) return <div className="py-12 text-center text-gray-500">Loading…</div>;
    ...
    return (
      <div className="mx-auto max-w-sm space-y-5 py-16">
        ...
        <p className="text-center text-xs text-gray-400">
          This is a temporary gate. Replace with real authentication before production.
        </p>
      </div>
    );
  }
