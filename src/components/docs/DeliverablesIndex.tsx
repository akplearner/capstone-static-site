...
function IndexTable({ courseId, roles }: { courseId: string; roles?: RoleDef[] }) {
  const defs = deliverablesForCourse(courseId);
  const ownerName = (id: string) => roles?.find((r) => r.id === id)?.name ?? id.toUpperCase();
  return (
        ...
        <tbody>
          {defs.map((d) => (
            <tr key={d.id} className="border-b border-gray-100 dark:border-gray-700/60">
              ...
              <td className="py-1.5 pr-3">{ownerName(d.owner)}</td>
    ...
    return (
      <div className="rounded-lg border border-gray-200 bg-white px-5 dark:border-gray-700 dark:bg-gray-800">
        <Collapsible title={`The ${count} deliverables — who owns what, and when`} defaultOpen={false}>
          <IndexTable courseId={courseId} roles={roles} />
        </Collapsible>
      </div>
    );
  }
  return <IndexTable courseId={courseId} roles={roles} />;
}
