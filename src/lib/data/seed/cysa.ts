    ...
    steps: [
      {
        ...
        instruction: 'In the dashboard: Security events › the Dashboard sub-tab (the graphs, not the raw event table) — its "Top 5 rule groups" and "Alerts evolution" panels give you real counts. To count any field yourself, open one event, expand a field like rule.description, and click Visualize for a bar chart. Filter to one machine with agent.name.',
        ...
        files: [
          { name: 'Wazuh dashboard — Security events (docs)', purpose: 'the Events table vs the Dashboard visualisations, and how to Visualize a field', source: 'https://documentation.wazuh.com/current/user-manual/wazuh-dashboard/index.html' },
        ],
        frameworks: ['NIST_CSF'],
      },
      {
        ...
        commands: [
          ...
          { cmd: 'data.alert.signature:*SQL*', explain: 'A SQL-injection web attack, from Suricata’s signature name. (If Apache logs are ingested, data.url:*union* works too.)' },
        ],
    ...
    steps: [
      ...
      },
      {
        ...
        commands: [
          ...
          { cmd: 'data.dest_port / data.alert.signature / agent.name', explain: 'Read these on the results: data.dest_port = which ports it hit (many in seconds = a scan), data.alert.signature = the Suricata rule name, agent.name = which of your machines.' },
        ],
      ...
      },
      {
        ...
        instruction: 'Copy the pcap to Kali (Wireshark is pre-installed there) with the scp line below, or to your own PC, then open it. In Wireshark: right-click a packet → Follow → HTTP Stream. Useful filters below; Statistics → Conversations is the fastest way to prove the port scan.',
        commands: [
