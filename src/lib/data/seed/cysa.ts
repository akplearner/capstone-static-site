    ...
    steps: [
      {
        ...
        whatItMeans: 'Green/Active means the agent is reporting. "Disconnected" or "Never connected" means the SOC has a blind spot on your host \u2014 first checks: is the agent service running, can the machine reach 10.10.100.100 on ports 1514/1515, and is the agent name unique (duplicate names are the #1 cause of a missing agent)?',
        ...
        isEvidenceStep: true,
        frameworks: ['NIST_CSF'],
        troubleshooting: 'Agent missing or Disconnected? On the host confirm the service is running (systemctl status wazuh-agent), that it can reach 10.10.100.100 on 1514/1515, and that its name is unique. Check that 10.10.100.100 is set as the manager in the agent’s ossec.conf.',
      },
      {
        id: 'cb-w1-s2',
        where: 'Wazuh dashboard',
...
/tmp/week2.pcap: OK`,
  ...
  // ─────────────────────────── WEEK 3 · ASSESS ───────────────────────────
  {
    ...
    handoff: [{ to: 'grc', artifact: 'SOC vulnerability list', note: 'Optional: share your SOC vuln list — a teammate can cross-check it against a Kali scan.' }],
    steps: [
      {
        id: 'cb-w3-s1',
        where: 'Wazuh dashboard',
      ...
      },
      {
        ...
        instruction: 'Run a quick recon scan from Kali against your OWN pod (e.g. `nmap <UBUNTU_IP>`), then open Security events, set the time picker to Last 15 minutes, and search the Kali IP. (If a teammate is scanning, you can watch theirs instead — but you don’t need to wait for anyone.)',
        commands: [
  ...
  },
  {
    ...
    handoff: [{ to: 'red', artifact: 'Confirmed vulns', note: 'Optional: share your confirmed vulns — a teammate can fold them into their risk ranking.' }],
    steps: [
      {
        id: 'cg-w3-s1',
        where: 'Kali',
...
/home/student/team-artifacts/week-4/access.log: OK`,
      ...
      },
      {
        ...
        instruction: 'Fill the Incident Response Report — the timeline table seeds example rows, so fill it from your own SOC search (or use a teammate’s timeline if they shared one), then add the evidence, root cause, fixes, and a 5-sentence manager summary (the form has worked example rows to copy). Root cause = the one weakness that let it in: trace the timeline back to the first successful step (e.g. "DVWA SQL-injection on an unpatched app with no WAF"), not just the last alert. The fixes are the remediations from the Vulnerability Assessment (03) that would have blocked that root cause.',
        usesForm: 'Incident Response Report',
