...
const tasks: Task[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        title: 'Log into the SOC dashboard',
        description: 'Confirm you can reach the shared SOC in a browser.',
        instruction: 'Open http://10.10.100.100 in a browser and sign in — username student, password @Pass@2026 (your shared class login). The SOC is already set up and running.',
  ...
  // ─────────────────────────── WEEK 1 · DEPLOY ───────────────────────────
  {
    ...
    prerequisites: ['The SOC is up at http://10.10.100.100 (sign in: student / @Pass@2026)', 'Your Ubuntu server and Windows PC already exist (DVWA + the OS were set up for you)', 'You can SSH into your Ubuntu pod as the `student` user', 'Your team\u2019s pod IPs (set them in the Lab access panel)'],
    ...
    steps: [
      ...
      },
      {
        ...
        commands: [
          ...
          { cmd: '<localfile>\n  <log_format>json</log_format>\n  <location>/var/log/suricata/eve.json</location>\n</localfile>', explain: 'Open /var/ossec/etc/ossec.conf (e.g. `nano /var/ossec/etc/ossec.conf`) and paste this block inside <ossec_config> \u2014 it ships Suricata\u2019s network alerts to the SOC.' },
      ...
      },
      {
        ...
        commands: [
          ...
          { cmd: 'dir Sysmon64.exe, sysmonconfig.xml', explain: 'Confirm BOTH files are in the folder before you install. If either is missing, re-run its download line above.' },
      ...
      },
      {
        ...
        instruction: 'Still in PowerShell as Administrator, in the same wazuh-lab folder: download the Wazuh agent MSI (link below), install it pointed at 10.10.100.100, add the Sysmon block to ossec.conf, then start the service.',
        commands: [
          { cmd: 'cd $HOME\\Downloads\\wazuh-lab', explain: 'Run the installer from the folder that holds the .msi you downloaded.' },
          { cmd: 'msiexec.exe /i wazuh-agent-4.14.7-1.msi /q WAZUH_MANAGER="10.10.100.100" WAZUH_AGENT_NAME="Team<#>-win"', explain: 'Install silently, pointed at your SOC and named for your team. Use the exact filename you downloaded — type wazuh-agent and press Tab to complete the version.' },
          { cmd: '<localfile>\n  <location>Microsoft-Windows-Sysmon/Operational</location>\n  <log_format>eventchannel</log_format>\n</localfile>', explain: 'Open C:\\Program Files (x86)\\ossec-agent\\ossec.conf (as Administrator) and paste this block inside <ossec_config> — it ships Sysmon\u2019s events to the SOC.' },
          { cmd: 'NET START WazuhSvc', explain: 'Start the Wazuh service after the install and config edit.' },
        ...
        frameworks: ['NIST_CSF'],
        troubleshooting: 'No "Connected to the server" in ossec.log? Most often two agents share a name — make it unique (Team<#>-win). Also confirm ossec.conf points to 10.10.100.100 (ports 1514 data / 1515 enrolment) and that the service is running (NET START WazuhSvc).',
      },
    ],
  },
  {
