  ...
  // ─────────────────────────── WEEK 1 · DEPLOY ───────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        whatItMeans: 'Suricata sniffs one network card. Its default config says eth0 — if you skip the NIC fix, Suricata runs but sees nothing.',
      ...
      },
      {
        ...
        commands: [
          ...
          { cmd: 'curl -s https://packages.wazuh.com/key/GPG-KEY-WAZUH | apt-key add -', explain: 'Trust the Wazuh signing key so packages verify.' },
          { cmd: 'echo "deb https://packages.wazuh.com/4.x/apt/ stable main" | tee -a /etc/apt/sources.list.d/wazuh.list', explain: 'Add the Wazuh 4.x package repository.' },
          ...
          { cmd: '# Add this block inside <ossec_config> in /var/ossec/etc/ossec.conf:', explain: 'This is what makes Suricata\u2019s network alerts reach the SOC.' },
          { cmd: '<localfile>\n  <log_format>json</log_format>\n  <location>/var/log/suricata/eve.json</location>\n</localfile>', explain: 'Tells the agent to ship Suricata\u2019s alerts to the SOC.' },
      ...
      },
      {
        ...
        frameworks: ['NIST_CSF'],
        troubleshooting: 'No “Sysmon” service after install? Run PowerShell as Administrator, make sure sysmon.exe and sysmonconfig.xml are in the same folder you cd’d into, and re-run `sysmon -accepteula -i sysmonconfig.xml`.',
      },
      {
        id: 'cr-w1-s5',
        where: 'Your Windows 11 PC',
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        instruction: 'Your notes: a short table covering all three feeds — host agent, Suricata (network), and Sysmon (Windows) — with whether data was present for each.',
        whatItMeans: 'A missing source is worth marks — write it up rather than ignore it.',
        expectedOutput: 'A short table: each of the three feeds checked, and whether data was present.',
    ...
    steps: [
      {
        ...
        instruction: 'In the dashboard: Agents › your agent › Security events. Group/read by rule.description (the Rule column — what fired), and note rule.groups (the category, e.g. authentication, ids, web) and agent.name (which machine, Ubuntu or Windows). Add these as columns from the field list on the left if they are not shown.',
        whatItMeans: 'Grouping by rule.description turns hundreds of rows into a few behaviours; rule.groups tells you the kind of activity and agent.name tells you which machine.',
        expectedOutput: 'You can name the top three alert types (by rule.description), how many of each, and which machine they came from.',
        frameworks: ['NIST_CSF'],
      },
      {
        id: 'cb-w2-s2',
        ...
        instruction: 'In the dashboard search bar, start with the severity cut, then try each attack-signature query below. (Full cheat sheet: "Filtering & what to look for" on the Guide.)',
        commands: [
          { cmd: 'rule.level:>=7', explain: 'Wazuh scores every alert 0–15; level 7 and up is where real activity usually is.' },
          { cmd: 'rule.groups:authentication_failed', explain: 'The tell for a brute force: many failed logins from one data.srcip in a short window.' },
          { cmd: 'rule.groups:ids', explain: 'Suricata network alerts. A burst from one data.srcip across many data.dstport = a port scan.' },
          { cmd: 'full_log:*union*', explain: 'A SQL-injection string in the request = a web attack (try *union*, *../*, *<script* too).' },
        ],
        whatItMeans: 'rule.level:>=7 shortens the list; the group/full_log queries tell a port scan from a brute force from a web attack instead of guessing.',
        expectedOutput: 'A short list, plus you can say which of the three shapes (scan / brute force / web attack) you actually see.',
        files: [
          ...
          { name: 'Wazuh dashboard search (docs)', purpose: 'how field:value queries and filters work in the search bar', source: 'https://documentation.wazuh.com/current/user-manual/wazuh-dashboard/queries.html' },
        ],
        frameworks: ['NIST_CSF'],
      },
      {
    ...
    steps: [
      {
        ...
        instruction: 'In the dashboard search bar, search by the suspicious source IP, then read the detail fields to size up what it did.',
        commands: [
          { cmd: 'data.srcip:<suspicious-ip>', explain: 'Shows everything that host did, not just what set off one alert.' },
          { cmd: 'data.dstport / data.url / agent.name', explain: 'Read these fields on the results: data.dstport = which ports it hit, data.url = which pages, agent.name = which of your machines. Many distinct dstports in seconds = a scan.' },
        ],
        whatItMeans: 'One source, all of its activity — the shape of the behaviour, not a single alert.',
        expectedOutput: 'You can say how many different ports (data.dstport) or URLs (data.url) the source hit, and on which machine.',
        frameworks: ['NIST_CSF'],
      },
      {
        id: 'cg-w2-s2',
        ...
        instruction: 'On your Ubuntu server — SSH in. Install tcpdump if it is not already there, start the capture, press Ctrl+C to stop it, then hash it.',
        commands: [
      ...
      },
      {
        ...
        instruction: 'Copy the pcap to Kali (Wireshark is pre-installed there) or your own PC, then open it. In Wireshark: right-click a packet → Follow → HTTP Stream. Useful filters below.',
        commands: [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        instruction: 'In your browser, paste each indicator into a free threat-intel site and record a verdict: use VirusTotal for files/URLs/domains and AbuseIPDB for IP addresses (both are free and need no login to search).',
        ...
        files: [
          ...
          { name: 'AbuseIPDB', purpose: 'check whether an IP address has been reported for abuse', source: 'https://www.abuseipdb.com' },
        ],
        producesDeliverable: '05_IOC_Database.csv',
        frameworks: ['NIST_CSF'],
      },
      {
        ...
        instruction: 'In the dashboard: MITRE ATT&CK. Open a tagged alert to see its technique ID (e.g. T1190) and confirm the technique matches what you saw.',
  ...
  // ─────────────────────────── WEEK 3 · ASSESS ───────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        instruction: 'Agree a scan time with the Threat Hunter. While they run the Kali scan, open Security events, set the time filter to Last 15 minutes, and search the Kali IP with the query below.',
        commands: [
          { cmd: 'data.srcip:<kali-ip>', explain: 'Show every alert coming from the scanning box, so you can see what the scan tripped.' },
        ],
