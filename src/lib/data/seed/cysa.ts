...
const tasks: Task[] = [
  // ─────────────────────────── WEEK 0 · Lab & SOC build (builder) ───────────────────────────
  {
    ...
    objective: 'Stand up the shared SOC and the pod templates, then clone. Every step is optional: if the SOC answers at 10.10.100.100, start at Week 1.',
    frameworks: ['NIST_CSF'],
    deliverables: [],
    ...
    steps: [
      {
        ...
        instruction: 'Set up the server side first — skip all of Week 0 if the SOC already answers at https://10.10.100.100. In the Proxmox console, give each VM a static IP.',
...
$ sudo systemctl status suricata --no-pager
     ...
     Notice: This is Suricata version 6.0.10 RELEASE running in SYSTEM mode`,
      ...
      },
      {
        ...
        instruction: 'Add both blocks inside <ossec_config> in /var/ossec/etc/ossec.conf on the Ubuntu template, then restart the agent. The Apache block makes the Week 2 web-attack fields work.',
        commands: [
      ...
      },
      {
        ...
        instructionList: [
          'On the SOC, confirm Vulnerability detection and SCA are enabled in ossec.conf — on Wazuh 4.8+ both are on by default.',
          'On the Ubuntu template, add a syscheck directory for the DVWA web root — this makes Week-4 file-change evidence possible.',
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        instruction: 'Read your Tier 2 mission on the course overview: the SOC Analyst escalates alerts to you; you prove them in logs and packets and hand findings to the Incident Responder.',
    ...
    steps: [
      ...
      },
      {
        ...
        instructionList: [
          ...
          'Prefer clicking? The download links are in the guide box below — save both into wazuh-lab, then run the install line.',
        ],
        commands: [
...
2026/08/05 09:31:06 wazuh-agent: INFO: Started module 'logcollector'.`,
      ...
      },
      {
        ...
        instructionList: [
          ...
          'Paste the `Connected to the server` line from ossec.log — that, not the service state, proves it reached the SOC.',
    ...
    steps: [
      {
        ...
        instructionList: [
          ...
          'Set BOTH the manager address and an agent name; an unnamed agent enrols under its hostname and gets lost.',
...
2026/07/22 09:11:44 wazuh-agentd: INFO: Connected to the server (10.10.100.100:1514/tcp).`,
      ...
      },
      {
        ...
        instruction: 'In the dashboard, open Agents (or Endpoints) and confirm your Team<#>-ubuntu agent shows Active with a recent Last keep alive. Click it for the exact time, then screenshot the list.',
      ...
      },
      {
        ...
        instructionList: [
          'The 3 routine alert types: Security events › Dashboard › "Top 5 rule groups" (or add rule.description as a column).',
  ...
  },
  {
    ...
    objective: 'Confirm every data source is reporting — host agent, Suricata network alerts and Sysmon Windows events. A missing source is itself a finding.',
    frameworks: ['NIST_CSF'],
    ...
    steps: [
      {
        ...
        instructionList: [
          ...
          'Run `ip -br addr` and note your real card name — on Proxmox it is usually ens18, not eth0.',
...
-rw-r----- 1 root root 18452 Jul 22 09:04 /var/log/suricata/eve.json`,
      ...
      },
      {
        ...
        instructionList: [
          ...
          'SCA and Vulnerabilities run on a schedule, so a new agent may show nothing for one cycle (15–60 min).',
        ],
...
{"timestamp":"2026-08-05T09:44:19.402881+0000","flow_id":1885329447,"event_type":"alert","src_ip":"10.10.30.7","src_port":51422,"dest_ip":"10.10.100.7","dest_port":80,"proto":"TCP","alert":{"signature":"ET SCAN Nmap Scripting Engine User-Agent Detected","category":"Web Application Attack","severity":2}}`,
      ...
      },
      {
        ...
        instruction: 'Fill the Coverage Validation Report (it has the columns and a worked example row). One row per feed: was data present, where did you check, what gap did you find?',
        instructionList: [
          'Host agent.',
...
data.alert.signature:SQL*                        3 hits   → web attack (SQLi)`,
      ...
      },
      {
        ...
        instruction: 'Fill the Alert Triage Report (it has the columns and a worked example row). One row per alert: count and severity, a verdict, a one-line reason, and whether you escalated.',
        instructionList: [
...
^C   (Ctrl+C after ~20s — you do not need it to finish)`,
      ...
      },
      {
        ...
        instruction: 'SSH into your Ubuntu server and start the capture, then re-run the Kali attack (step 1) so there is traffic. Ctrl+C to stop, take ownership of the file, hash it.',
        commands: [
    ...
    steps: [
      {
        ...
        instruction: 'Fill the IOC Database (it has the columns and worked example rows). Each row is type (IP · domain · URL · hash), value, where you first saw it, and a timestamp.',
        instructionList: [
    ...
    steps: [
      {
        ...
        instruction: 'In the dashboard: Agents › your agent › Vulnerabilities. Filter Severity to Critical + High, screenshot the list as 20260726_Team07_wazuh_vulns.png, and hand it to the Incident Responder.',
      ...
      },
      {
        ...
        instruction: 'In the dashboard: Agents › your agent › SCA. Filter Result to Failed, click any failed check to read its Rationale and Remediation, then screenshot the score and three failed checks.',
      ...
      },
      {
        ...
        instructionList: [
          'One row per finding: source (Vulnerabilities or SCA), what it is, severity or result, and the module’s stated fix.',
...
+ 1 host(s) tested`,
      ...
      },
      {
        ...
        instructionList: [
          'One row per service: the nmap version from your -sV output, the Wazuh package version from the Vulnerabilities module.',
    ...
    steps: [
      {
        ...
        instruction: 'Fill the Vulnerability Assessment (it has the columns and a worked example row). Each finding needs a CVE and its CVSS score, or a written reason where there is none.',
        instructionList: [
          'To find the CVE, paste the software and version into NVD search.',
          'Use the package + version from Wazuh Vulnerabilities (e.g. "openssl 1.1.1f") or the nmap -sV banner (e.g. "Apache httpd 2.4.41").',
      ...
      },
      {
        ...
        instructionList: [
          ...
          'Likelihood = how reachable: High, from the network with no login; Medium, needs an account or a click; Low, physical access.',
          'Impact = what the attacker gets: High, full data loss or admin; Medium, one service down or limited exposure; Low, cosmetic.',
          'The risk matrix on the Deliverables page draws these axes as a grid: read the cell where your ratings meet.',
        ],
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        instruction: 'Fill the Executive Debrief and Lessons Learned form (it has the sections and a worked example): a plain-English summary, the headline metrics, and lessons and recommendations with owners.',
        instructionList: [
...
/home/student/team-artifacts/week-4/access.log: OK`,
      ...
      },
      {
        ...
        instructionList: [
          ...
          'Root cause = the one weakness that let it in: the first successful step in the timeline, not the last alert.',
