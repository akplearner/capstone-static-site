  ...
  // ─────────────────────────── WEEK 1 · DEPLOY ───────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        instruction: 'On your Windows 11 PC: open PowerShell as Administrator, download Sysmon and a sane default config, then install it.',
        commands: [
          { cmd: '# Download Sysmon from the Sysinternals link below and the SwiftOnSecurity sysmonconfig.xml', explain: 'Sysmon logs almost nothing without a config.' },
          { cmd: 'sysmon -accepteula -i sysmonconfig.xml', explain: 'Install Sysmon as a service using the standard config.' },
        ...
        expectedOutput: 'Sysmon appears in Windows Services as Running, and the Microsoft-Windows-Sysmon/Operational log has events.',
        files: [
          { name: 'Sysmon (Sysinternals)', purpose: 'the rich Windows event source the agent forwards', source: 'https://learn.microsoft.com/sysinternals/downloads/sysmon' },
          { name: 'sysmonconfig.xml', purpose: 'a sane default config — Sysmon logs almost nothing without one', source: 'https://github.com/SwiftOnSecurity/sysmon-config' },
        ],
        frameworks: ['NIST_CSF'],
        troubleshooting: 'No “Sysmon” service after install? Run PowerShell as Administrator, make sure sysmon.exe and sysmonconfig.xml are in the same folder, and re-run `sysmon -accepteula -i sysmonconfig.xml`.',
      },
      {
        id: 'cr-w1-s5',
        where: 'Your Windows 11 PC',
        ...
        expectedOutput: 'Windows Services shows Wazuh as Running. (No firewall step here — Windows Defender Firewall is already on; UFW is Linux-only.)',
        files: [
