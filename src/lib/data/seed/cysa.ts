  ...
  // ─────────────────────────── WEEK 1 · DEPLOY ───────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        instruction: 'On your Windows 11 PC, open PowerShell as Administrator. Make one folder for the lab and download BOTH files into it: Sysmon (Sysinternals) and the SwiftOnSecurity sysmonconfig.xml (links below). Then cd into that folder and install.',
        commands: [
          { cmd: 'mkdir $HOME\\Downloads\\wazuh-lab; cd $HOME\\Downloads\\wazuh-lab', explain: 'Make and enter the lab folder. Put Sysmon64.exe and sysmonconfig.xml here.' },
    ...
    steps: [
      {
        ...
        expectedOutput: 'A list of open ports with service names and versions, saved to nmap_<team>.txt.',
        verify: ['open'],
        files: [
          { name: 'nmap', purpose: 'the port/version scanner (pre-installed on Kali)', source: 'sudo apt install -y nmap' },
        ],
        troubleshooting: 'Scan shows all ports filtered or the host down? You’re scanning the wrong pod or a firewall is dropping you — confirm you can `ping <UBUNTU_IP>` first, and only scan your own team’s pod (scope rule).',
      ...
      },
      {
        ...
        expectedOutput: 'At least one Nikto finding with a reference ID, saved to nikto_<team>.txt.',
        verify: ['+'],
        files: [
      ...
      },
      {
        ...
        instruction: 'In your notes, line the Kali findings up against the SOC\u2019s Vulnerabilities list. They differ because Wazuh reads installed package versions while nmap reads the live service banner \u2014 note each difference and which source you trust.',
    ...
    steps: [
      {
        ...
        instruction: 'Fill the Vulnerability Assessment: each finding with a CVE + CVSS score (look the CVE up on NVD; score it with the CVSS calculator), or a written reason where there is no CVE.',
      ...
      },
      {
        ...
        instruction: 'In the Vulnerability Assessment: plot each finding by likelihood × impact, then give each a fix, an owner and a target date (SCA supplies fix text for config findings).',
        ...
        expectedOutput: 'Every finding is ranked by risk and carries a fix, an owner and a date.',
        frameworks: ['CVSS', 'NIST_800_115'],
      },
    ],
  },
