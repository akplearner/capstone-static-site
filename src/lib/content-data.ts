...
const BLUE_TASKS: Task[] = [
  {
    ...
    frameworks: ['NIST_CSF'],
    deliverables: [],
    learn: ['Connecting to Linux (SSH) and Windows (RDP) hosts', 'Taking a baseline snapshot before any change', 'Running a containerized web app with Docker (DVWA)'],
    ...
    definitionOfDone: ['You can SSH into the Ubuntu target', 'You can RDP into the Windows target', 'A "pre-hardening" snapshot exists for both VMs', 'DVWA is running and reachable at http://<UBUNTU_IP>/'],
    handoff: [
    ...
    steps: [
      ...
      },
      {
        ...
        commandFlags: [
          ...
          { flag: 'systemctl enable --now docker', meaning: 'Start Docker now and on every boot.' },
          ...
          { flag: 'curl -I http://localhost/', meaning: 'Fetch only the headers to confirm the web app answers.' },
        ],
        expectedOutput: 'HTTP/1.1 302 Found (redirect to /login.php); `docker ps` shows the container running',
        outputExplanation: 'A 200/302 response means DVWA is serving; browse to http://<UBUNTU_IP>/ and you should see the DVWA login page. Default login is admin / password.',
        whatItMeans: 'The vulnerable target is live, so Red has something authorized to test and Blue has a real service to defend.',
        frameworks: ['NIST_CSF'],
        troubleshooting: 'Port 80 already in use? Stop the host web server (`sudo systemctl stop apache2`) or map another port, e.g. `-p 8080:80`. First run pulls the image, so give it a minute.',
      },
    ],
  },
  {
