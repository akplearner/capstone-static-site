    ...
    steps: [
      {
        ...
        instruction: 'In a browser, log into DVWA, set Security = Low, and open the SQL Injection page once so the id parameter exists. Then run sqlmap from Kali against that URL.',
        commands: [
...
+---------+---------+----------------------------------+`,
      ...
      },
      {
        ...
        instructionList: [
          'login.php has a per-request anti-CSRF token hydra cannot submit, so a password it "finds" there will not log you in.',
...
1 of 1 target successfully completed, 1 valid password found`,
      ...
      },
      {
        ...
        instructionList: [
          'DVWA’s container usually lacks nc and bash, so use the PHP payload (command 4); 5 and 6 are fallbacks.',
          'Kali terminal: start the listener (command 2) and leave it running.',
          'In DVWA (Security = Low) → Command Injection, run command 3 to see what exists, then submit ONE payload (4, else 5/6).',
...
dvwa-target`,
      ...
      },
      {
        ...
        instructionList: [
          ...
          'In DVWA (Security = Low) → File Upload, upload shell.php, note the path DVWA prints, then open it (command 4).',
