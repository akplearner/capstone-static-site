    ...
    steps: [
      ...
      },
      {
        ...
        commands: [
          ...
          { cmd: 'sudo docker run -d --name dvwa --restart unless-stopped -p 80:80 vulnerables/web-dvwa', explain: 'Create + run the DVWA container named "dvwa" in the background on host port 80.' },
          { cmd: 'curl -I http://localhost/', explain: 'Fetch the headers to confirm DVWA responds.' },
        ],
        commandExplanation: 'Installs Docker, runs the DVWA image as a named background container mapping host port 80 to the app, then curls the host to confirm DVWA responds.',
        commandFlags: [
          { flag: 'docker run -d', meaning: 'Run the container in the background (detached).' },
          { flag: '--name dvwa', meaning: 'Give the container a fixed name so you can start/stop it by name later.' },
          { flag: '--restart unless-stopped', meaning: 'Bring DVWA back up automatically after a reboot.' },
          { flag: '-p 80:80', meaning: 'Map host port 80 to the container so DVWA is reachable over HTTP.' },
          { flag: 'vulnerables/web-dvwa', meaning: 'The DVWA (Damn Vulnerable Web App) image to run.' },
        ],
