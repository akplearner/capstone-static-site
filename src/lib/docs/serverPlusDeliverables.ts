    ...
    sections: [
      ...
      },
      {
        ...
        fields: [
          ...
          { field: 'remote_routes', label: 'Zones reachable through the host (Week 3)', type: 'text', placeholder: '172.16.0.0/24 and 192.168.0.0/24 approved; SSH to websrv and linuxsrv from home', help: 'Set in Week 3, when the host becomes a Tailscale subnet router. Until then, write "host only".' },
      ...
      },
      {
        ...
        group: {
          ...
          seed: [
            ...
            { from: 'websrv', to: 'an internet name, through winserver', command: `nslookup archive.ubuntu.com ${vm('winserver').address}`, expected: 'Should work', result: 'Answered — the forwarder on winserver works', matches: 'Yes' },
            ...
            { from: 'Tailnet laptop', to: 'websrv and linuxsrv by zone address', command: `ssh ubuntu@${vm('websrv').address} hostname && ssh ubuntu@${vm('linuxsrv').address} hostname`, expected: 'Should work', result: 'Both hostnames print — subnet routes through the host', matches: 'Yes' },
          ],
        },
      },
      {
      ...
      },
      {
        ...
        group: {
          ...
          seed: [
            ...
            { name: 'Remote access is down', trigger: 'You cannot reach the server from off campus', who: 'IT administrator', steps: '1) Check your own laptop is signed in and connected to the tailnet 2) From on campus, reach the host on its team address and log in at the console 3) systemctl is-active tailscaled — start and enable it if it is not active 4) tailscale status — the host must list as online 5) If it was rebuilt and re-authorised as a new device, record the new address 6) Re-test SSH and the console from off campus 7) Record what it was', verify: 'SSH and the console on port 8006 both answer from a device that is not on the campus network', rollback: 'Administer from the campus LAN on the team address while it is down — the build never depended on remote access to keep running' },
    ...
    sections: [
      ...
      },
      {
        ...
        group: {
          ...
          columns: [
            c('tool', 'Tool', 'select', { options: ['Terraform', 'OpenTofu', 'Prometheus', 'Grafana', 'Loki', 'Pulse', 'Wazuh', 'NetBox', 'GLPI', 'Gitea', 'Ansible', 'Proxmox Backup Server', 'Alertmanager'] }),
            c('host', 'Runs on', 'hostref', { placeholder: 'secmon', help: 'The hostname from the Architecture Brief.' }),
            c('url', 'Open it at', 'text', { placeholder: 'http://192.168.0.4:3000', help: 'The address a person types. Terraform and OpenTofu have none — write "workstation".' }),
