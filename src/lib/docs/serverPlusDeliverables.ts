    ...
    sections: [
      {
        ...
        group: {
          ...
          help: 'At least four planned VMs. Build order says what exists by Week 2 and what is planned for later.',
          columns: [
            c('vm', 'VM name', 'text', { placeholder: 'winserver' }),
            c('role', 'Server role', 'select', { options: ['Directory services', 'File server', 'Application server', 'Database server', 'Security / monitoring', 'Other'] }),
          ...
          seed: [
            { vm: 'winserver', role: 'Directory services', os: 'Windows Server 2022', resources: '2 vCPU / 4 GB / 60 GB', build: 'Week 2 (core)' },
            { vm: 'linuxsrv', role: 'Database server', os: 'Ubuntu Server 22.04', resources: '2 vCPU / 4 GB / 40 GB', build: 'Week 2 (core)' },
            { vm: 'filesrv', role: 'File server', os: 'Windows Server 2022', resources: '2 vCPU / 4 GB / 200 GB', build: 'Planned — later phase' },
            { vm: 'secmon', role: 'Security / monitoring', os: 'Ubuntu Server 22.04', resources: '2 vCPU / 6 GB / 80 GB', build: 'Planned — later phase' },
          ],
        },
      },
      {
      ...
      },
      {
        ...
        fields: [
          { field: 'physical', label: 'Physical path', type: 'area', placeholder: 'Office wall port → patch panel P1 → switch Gi0/1 → server NIC 1 (management).' },
          { field: 'virtual', label: 'Virtual layout', type: 'area', placeholder: 'Hypervisor host runs winserver and linuxsrv, both bridged to the office LAN.' },
      ...
      },
      {
        ...
        group: {
          ...
          help: 'Every host gets a row before it connects. This is the only place addresses are decided.',
          columns: [
          ...
          seed: [
            { hostname: 'pve-host', role: 'Hypervisor host', ip: '10.10.10.10', gateway: '10.10.10.1', dns: '—', assignment: 'Static' },
    ...
    sections: [
      {
        ...
        group: {
          ...
          columns: [
            ...
            c('baseline', 'Baseline configuration (values)', 'area', { placeholder: 'Hostname pve-host; IP 10.10.10.10/24; gw 10.10.10.1; bridge vmbr0 on NIC 1; no-subscription repo enabled.' }),
          ...
          seed: [
            { system: 'Hypervisor host', purpose: 'Virtualization platform', baseline: 'Hostname pve-host; IP 10.10.10.10/24; gateway 10.10.10.1; bridge vmbr0 on NIC 1; no-subscription repo enabled.', guide_ref: 'Config Guide §4 — Proxmox install', evidence: 'GranitePeak_Wk2_pve-config.png' },
            { system: 'winserver VM', purpose: 'Directory, DNS, DHCP', baseline: '2 vCPU / 4 GB / 60 GB; static 10.10.10.20; roles: AD DS, DNS, DHCP.', guide_ref: 'Config Guide §6 — Windows roles', evidence: 'GranitePeak_Wk2_win-roles.png' },
